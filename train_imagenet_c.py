...
label2id = utils_pytorch.get_label2id("/scratch/project_2006532/dataset/ImageNet-c/split_{}/label_name.txt".format(args.protocol))

trainset_data, trainset_targets = utils_pytorch.get_data_file_imagenet("/scratch/project_2006532/dataset/ImageNet-c/split_1/train.txt",
                                                              "/scratch/project_2006532/dataset/ImageNet-c/",
                                                              label2id)
...
class_means = np.zeros((768, 100, 2))

output_dir = '/scratch/project_2006532/imagenet_result_log'
...
for session in range(start_session, 18):

    #------traning and testing data in the current session------
    train_file = os.path.join(args.data_dir, args.dataset, "split_{}".format(args.protocol), "session_{}.txt".format(session - 4))
    test_file = os.path.join(args.data_dir, args.dataset, "split_{}".format(args.protocol), "test_{}.txt".format(session - 4))
    ...
    X_train, Y_train = utils_pytorch.get_data_file_imagenet(train_file, "/scratch/project_2006532/dataset/ImageNet-c/", label2id)
    X_valid,  Y_valid = utils_pytorch.get_data_file_imagenet(test_file, "/scratch/project_2006532/dataset/ImageNet-c/", label2id)
    ...
    if args.resume == False:
        if session == start_session:
            ckp_name = '/scratch/project_2006532/checkpoint/imagenet_vit_{}_epochs_{}_iteration_{}_model.pth'.format(args.dataset, args.epochs, session)
            torch.save(tg_model, ckp_name)
    ...
    class_means_current = current_means[:, -10:, 0]
    ...
    unseen_test_file = os.path.join(args.data_dir, args.dataset, "split_{}".format(args.protocol), "unseen.txt")
    X_unseen, Y_unseen = utils_pytorch.get_data_file_imagenet_unseen(classes_current, unseen_test_file, "/scratch/project_2006532/dataset/ImageNet-c/", label2id)
    ...
    X_unseen_base, Y_unseen_base = utils_pytorch.get_data_file_imagenet_unseen(classes_base, unseen_test_file, "/scratch/project_2006532/dataset/ImageNet-c/", label2id)
