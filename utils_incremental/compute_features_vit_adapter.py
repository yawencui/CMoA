...
def compute_features(tg_model, evalloader, num_samples, num_features, device=None):
    #if device is None:
    ...
    with torch.no_grad():
        for inputs, targets in evalloader:
            ...
            features_vit, logits = tg_model(inputs)
            features[start_idx:start_idx+inputs.shape[0], :] = np.squeeze(features_vit.data.cpu().numpy())
